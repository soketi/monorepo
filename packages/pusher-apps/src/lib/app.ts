...
import { schema, type AppSchema, type AppSchemaKey } from './schema';
import type { WebhookInterface } from './webhook';
import { toOrderedArray, getMD5, PusherToken } from '@soketi/utils';
...
export class App implements Partial<AppSchema> {
  ...
  constructor(protected env?: Record<string, any>) {
    //
  }
  ...
  static async load(
    ...
    for await (const key of defKeys) {
      ...
      for (const parameter of def.parameters) {
        let value: AppSchema[AppSchemaKey] =
          (object[
            parameter as AppSchemaKey
          ] as unknown as AppSchema[AppSchemaKey]) ||
  ...
  async createToken(params: string): Promise<string> {
    return new PusherToken(this.key as string, this.secret as string).sign(
      params,
    );
  }
  ...
  async calculateRequestToken(
    ...
    return await this.createToken(
      [method, path, toOrderedArray(params).join('&')].join('\n'),
    );
  }

  async calculateSigninToken(
    connId: string,
    userData: string,
  ): Promise<string> {
  ...
  async signinTokenIsValid(
    connId: string,
    userData: string,
    receivedToken: string,
  ): Promise<boolean> {
    return (
      `${this.key}:${await this.calculateSigninToken(connId, userData)}` ===
      receivedToken
    );
  }
}
