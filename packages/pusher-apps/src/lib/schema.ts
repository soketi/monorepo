...
>(potentialJson: Input): Promise<Output> | Output {
  ...
  try {
    const potentialArray = JSON.parse(potentialJson as string);
...
export interface AppLoadSchemaInput<Output, Input = any> {
  default: Output | ((env?: Record<string, any>) => Output | Promise<Output>);
  parameters: string[];
  ...
  webhooks: {
    ...
    parsers: [
      transformPotentialJsonToArray<
        string | WebhookInterface[],
        WebhookInterface[]
      >,
    ],
  },
  maxPresenceMembersPerChannel: {
    default: (env?: Record<string, any>) =>
      env?.['PRESENCE_MAX_MEMBERS'] || 100,
    parameters: ['maxPresenceMembersPerChannel'],
    parsers: [parseInt],
  },
  maxPresenceMemberSizeInKb: {
    default: (env?: Record<string, any>) =>
      env?.['PRESENCE_MAX_MEMBER_SIZE'] || 2,
    parameters: ['maxPresenceMemberSizeInKb'],
    parsers: [parseFloat],
  },
  maxChannelNameLength: {
    default: (env?: Record<string, any>) =>
      env?.['CHANNEL_LIMITS_MAX_NAME_LENGTH'] || 200,
    parameters: ['maxChannelNameLength'],
    parsers: [parseInt],
  },
  maxEventChannelsAtOnce: {
    default: (env?: Record<string, any>) =>
      env?.['EVENT_MAX_CHANNELS_AT_ONCE'] || 100,
    parameters: ['maxEventChannelsAtOnce'],
    parsers: [parseInt],
  },
  maxEventNameLength: {
    default: (env?: Record<string, any>) =>
      env?.['EVENT_MAX_NAME_LENGTH'] || 200,
    parameters: ['maxEventNameLength'],
    parsers: [parseInt],
  },
  maxEventPayloadInKb: {
    default: (env?: Record<string, any>) =>
      env?.['EVENT_MAX_SIZE_IN_KB'] || 100,
    parameters: ['maxEventPayloadInKb'],
    parsers: [parseFloat],
  },
  maxEventBatchSize: {
    default: (env?: Record<string, any>) => env?.['EVENT_MAX_BATCH_SIZE'] || 10,
