...
> extends Server<ConnectionsPoolT, BrainT, GossiperT> {
  server!: TemplatedApp;
  socket?: us_listen_socket;

  // TODO: Implement Router.
  constructor(options: ServerOptions<ConnectionsPoolT, BrainT, GossiperT>) {
    super({
      ...options,
    });
  ...
  override async start(signalHandler?: () => Promise<void>): Promise<void> {
    ...
    return new Promise(async (resolve) => {
      this.server.ws<WebSocketData<ConnectionsPool['connectionId']>>(
        '/:namespace',
        {
          ...
          open: async (ws) => {
            ...
            if (!this.connections.hasNamespace(namespace)) {
              ...
              await this.gossiper.subscribeToNamespace(
                namespace,
                async (data) => {
