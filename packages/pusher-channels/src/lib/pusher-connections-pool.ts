...
import { ConnectionsPool, Connection } from '@soketi/connections';
import { toString as uint8ArrayToString } from 'uint8arrays/to-string';
import { fromString as uint8ArrayFromString } from 'uint8arrays/from-string';
import { type Peer, type PeerData } from '@libp2p/interface/src/peer-store';
...
import { App, AppsManager } from '@soketi/pusher-apps';
import { PusherUserID } from './presence/pusher-user';
import {
  JsonStringifiable,
  dataToKilobytes,
...
export class PusherConnectionsPool<
  C extends PusherConnection<
    Connection['id'],
    AnyPusherEvent
  > = PusherConnection<Connection['id'], AnyPusherEvent>,
  Message extends JsonStringifiable = AnyPusherEvent,
> extends ConnectionsPool<Message, C> {
  readonly channels = new Map<App['id'], Map<string, Set<C['id']>>>();
  readonly users = new Map<PusherUserID, Set<C['id']>>();
  ...
  async namespaceApp(
    namespace: Required<App>['id'],
  ): Promise<Required<App> | undefined> {
  ...
  override async newConnection(conn: C): Promise<void> {
    ...
    await this.syncCurrentPeer({
      metadata: {
        [`pusher:app:${conn.namespace}:connections_count`]:
          uint8ArrayFromString(this.namespace(conn.namespace).size.toString()),
      },
      tags: {
  ...
  override async removeConnection(
    conn: C,
    ...
    if (conn.user) {
      ...
      if (
        this.users.get(conn.user.id) &&
        this.users.get(conn.user.id)?.size === 0
      ) {
  ...
  async subscribeToChannel(
    conn: C,
    message: AnyPusherSubscriptionEvent,
  ): Promise<any> {
    const app = await this.namespaceApp(conn.namespace);
    ...
    const channelManager = await this.getChannelManagerFor(channel, app);
    const maxChannelNameLength = app.maxChannelNameLength;
    ...
    const response = await channelManager.join(conn, channel, message as any);

    if (!response.success) {
      const { authError, type, errorMessage, errorCode } = response;
    ...
    if (!(channelManager instanceof PresenceChannelManager)) {
      ...
      await this.syncCurrentPeer({
        metadata: {
          ...
          [`pusher:app:${conn.namespace}:channel:${channel}:connections_count`]:
  ...
  async unsubscribeFromAllChannels(conn: C): Promise<void> {
    await Promise.allSettled(
      [...conn.subscribedChannels].map((channel) =>
  ...
  async unsubscribeFromChannel(conn: C, channel: string): Promise<void> {
    ...
    const channelManager = await this.getChannelManagerFor(channel, app);
    const response = await channelManager.leave(conn, channel);
  ...
  async handleSignin(conn: C, message: PusherSignin): Promise<void> {
    if (!conn.userAuthenticationTimeout) {
      return;
    }
  ...
  async broadcastJsonMessageToChannel<T extends PusherClientEvent>(
    ...
    const connections = this.getLocalChannelConnections(
      namespace,
      message.channel,
    );
  ...
  async getConnectionsCount(namespace: string): Promise<number> {
    ...
    const value = (peer: Peer) =>
      peer.metadata.get(`pusher:app:${namespace}:connections_count`) ||
  ...
  getLocalChannelConnections(namespace: string, channel: string): Set<C> {
    ...
    return new Set(
      [...cids]
        .map((id) => this.namespace(namespace)?.get(id))
  ...
  async getChannelsWithConnectionsCount(
    ...
    const value = (peer: Peer) =>
      peer.metadata.get(`pusher:app:${namespace}:channels`) ||
      uint8ArrayFromString('[]');
    ...
    for await (const channel of channels) {
      channelsWithConnections.set(
        channel,
        await this.getChannelConnectionsCount(channel),
      );
    }
  ...
  async getChannelManagerFor(
    ...
    if (await isPresenceChannel(channel)) {
      return new PresenceChannelManager(this, app);
    } else if (await isEncryptedPrivateChannel(channel)) {
      return new EncryptedPrivateChannelManager(this, app);
    } else if (await isPrivateChannel(channel)) {
